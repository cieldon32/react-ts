import type { Instance, TextInstance, SuspenseInstance, Container, ChildSet, UpdatePayload } from "./ReactFiberHostConfig";
...
import { captureCommitPhaseError, resolveRetryWakeable, markCommitTimeOfFallback, enqueuePendingPassiveProfilerEffect, restorePendingUpdaters } from "./ReactFiberWorkLoop.old";
...
function safelyCallCommitHookLayoutEffectListMount(current: Fiber, nearestMountedAncestor: Fiber | null) {
  try {
...
// Capture errors so they don't interrupt mounting.
function safelyCallComponentDidMount(current: Fiber, nearestMountedAncestor: Fiber | null, instance: any) {
  try {
...
function safelyCallDestroy(current: Fiber, nearestMountedAncestor: Fiber | null, destroy: () => void) {
  try {
    destroy();
  } catch (error) {
    reportUncaughtErrorInDEV(error);
...
function hideOrUnhideAllChildren(finishedWork, isHidden) {
  ...
  if (supportsMutation) {
    ...
    while (true) {
      ...
      } else if ((node.tag === OffscreenComponent || node.tag === LegacyHiddenComponent) && (node.memoizedState as OffscreenState) !== null && node !== finishedWork) {// Found a nested Offscreen component that is hidden.
...
function commitUnmount(finishedRoot: FiberRoot, current: Fiber, nearestMountedAncestor: Fiber): void {
    ...
    case DehydratedFragment:
      {
        if (enableSuspenseCallback) {
          ...
          if (hydrationCallbacks !== null) {
            ...
            if (onDeleted) {
              onDeleted((current.stateNode as SuspenseInstance));
            }
          }
        }
...
function commitNestedUnmounts(finishedRoot: FiberRoot, root: Fiber, nearestMountedAncestor: Fiber): void {
...
function unmountHostComponents(finishedRoot: FiberRoot, current: Fiber, nearestMountedAncestor: Fiber): void {
  ...
  while (true) {
    ...
    if (node.tag === HostComponent || node.tag === HostText) {
      ...
      if (currentParentIsContainer) {
        removeChildFromContainer(((currentParent as any) as Container), (node.stateNode as Instance | TextInstance));
      } else {
        removeChild(((currentParent as any) as Instance), (node.stateNode as Instance | TextInstance));
      ...
      if (enableSuspenseCallback) {
        ...
        if (hydrationCallbacks !== null) {
          ...
          if (onDeleted) {
            onDeleted((node.stateNode as SuspenseInstance));
          }
        }
      }
      ...
      if (currentParentIsContainer) {
        clearSuspenseBoundaryFromContainer(((currentParent as any) as Container), (node.stateNode as SuspenseInstance));
      } else {
        clearSuspenseBoundary(((currentParent as any) as Instance), (node.stateNode as SuspenseInstance));
      }
...
export function commitPassiveMountEffects(root: FiberRoot, finishedWork: Fiber): void {
  nextEffect = finishedWork;
...
function commitPassiveMountEffects_complete(subtreeRoot: Fiber, root: FiberRoot) {
  while (nextEffect !== null) {
    const fiber = nextEffect;
...
function commitPassiveMountOnFiber(finishedRoot: FiberRoot, finishedWork: Fiber): void {
  switch (finishedWork.tag) {
    case FunctionComponent:
    case ForwardRef:
    case SimpleMemoComponent:
...
export { commitResetTextContent, commitPlacement, commitDeletion, commitWork, commitAttachRef, commitDetachRef, invokeLayoutEffectMountInDEV, invokeLayoutEffectUnmountInDEV, invokePassiveEffectMountInDEV, invokePassiveEffectUnmountInDEV };