...
import { requestEventTime, requestUpdateLane, scheduleUpdateOnFiber, flushRoot, batchedUpdates, flushSync, isAlreadyRendering, flushControlled, deferredUpdates, discreteUpdates, flushPassiveEffects } from "./ReactFiberWorkLoop.old";
...
function getContextForSubtree(parentComponent: React.Component<any, any> | null | undefined): Record<string, any> {
  if (!parentComponent) {
...
function findHostInstance(component: Record<string, any>): PublicInstance | null {
...
function findHostInstanceWithWarning(component: Record<string, any>, methodName: string): PublicInstance | null {
  if (__DEV__) {
    const fiber = getInstance(component);
...
export function createContainer(containerInfo: Container, tag: RootTag, hydrate: boolean, hydrationCallbacks: null | SuspenseHydrationCallbacks, isStrictMode: boolean, concurrentUpdatesByDefaultOverride: null | boolean, identifierPrefix: string): OpaqueRoot {
...
export function updateContainer(element: ReactNodeList, container: OpaqueRoot, parentComponent: React.Component<any, any> | null | undefined, callback: Function | null | undefined): Lane {
  if (__DEV__) {
...
if (__DEV__) {
  const copyWithDeleteImpl = (obj: Record<string, any> | Array<any>, path: Array<string | number>, index: number) => {
  ...
  const copyWithRenameImpl = (obj: Record<string, any> | Array<any>, oldPath: Array<string | number>, newPath: Array<string | number>, index: number) => {
  ...
  const copyWithRename = (obj: Record<string, any> | Array<any>, oldPath: Array<string | number>, newPath: Array<string | number>): Record<string, any> | Array<any> => {
  ...
  const copyWithSetImpl = (obj: Record<string, any> | Array<any>, path: Array<string | number>, index: number, value: any) => {
  ...
  setErrorHandler = (newShouldErrorImpl: (arg0: Fiber) => boolean | null | undefined) => {
    shouldErrorImpl = newShouldErrorImpl;
  };

  setSuspenseHandler = (newShouldSuspendImpl: (arg0: Fiber) => boolean) => {
