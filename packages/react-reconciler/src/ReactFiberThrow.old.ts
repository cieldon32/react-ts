...
import { renderDidError, onUncaughtError, markLegacyErrorBoundaryAsFailed, isAlreadyFailedLegacyErrorBoundary, pingSuspendedRoot, restorePendingUpdaters } from "./ReactFiberWorkLoop.old";
...
function createRootErrorUpdate(fiber: Fiber, errorInfo: CapturedValue<unknown>, lane: Lane): Update<unknown> {
...
function createClassErrorUpdate(fiber: Fiber, errorInfo: CapturedValue<unknown>, lane: Lane): Update<unknown> {
  const update = createUpdate(NoTimestamp, lane);
  update.tag = CaptureUpdate;
...
function attachWakeableListeners(suspenseBoundary: Fiber, root: FiberRoot, wakeable: Wakeable, lanes: Lanes) {
...
function getNearestSuspenseBoundaryToCapture(returnFiber: Fiber) {
  ...
  const hasInvisibleParentBoundary = hasSuspenseContext(suspenseStackCursor.current, (InvisibleParentSuspenseContext as SuspenseContext));
...
function markSuspenseBoundaryShouldCapture(suspenseBoundary: Fiber, returnFiber: Fiber, sourceFiber: Fiber, root: FiberRoot, rootRenderLanes: Lanes): Fiber | null {
    ...
    if (suspenseBoundary === returnFiber) {
      ...
      if (supportsPersistence && enablePersistentOffscreenHostContainer) {
        ...
        if (currentSuspenseBoundary === null) {
          const offscreenFiber: Fiber = (suspenseBoundary.child as any);
...
function throwException(root: FiberRoot, returnFiber: Fiber, sourceFiber: Fiber, value: unknown, rootRenderLanes: Lanes) {
