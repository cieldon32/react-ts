...
import { enableSuspenseServerRenderer, enableLazyContextPropagation } from "shared/ReactFeatureFlags";
...
export function scheduleWorkOnParentPath(parent: Fiber | null, renderLanes: Lanes) {
...
function propagateContextChanges<T>(workInProgress: Fiber, contexts: Array<any>, renderLanes: Lanes, forcePropagateEntireTree: boolean): void {
...
export function lazilyPropagateParentContextChanges(current: Fiber, workInProgress: Fiber, renderLanes: Lanes) {
...
// have changed.
export function propagateParentContextChangesToDeferredTree(current: Fiber, workInProgress: Fiber, renderLanes: Lanes) {
  const forcePropagateEntireTree = true;
  propagateParentContextChanges(current, workInProgress, renderLanes, forcePropagateEntireTree);
}

function propagateParentContextChanges(current: Fiber, workInProgress: Fiber, renderLanes: Lanes, forcePropagateEntireTree: boolean) {
  if (!enableLazyContextPropagation) {
    return;
  }
