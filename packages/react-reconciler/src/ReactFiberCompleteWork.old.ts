...
import type { ReactScopeInstance, ReactContext, Wakeable } from "shared/ReactTypes";
import type { FiberRoot } from "./ReactInternalTypes";
import type { Instance, Type, Props, Container, ChildSet } from "./ReactFiberHostConfig";
import type { SuspenseState, SuspenseListRenderState } from "./ReactFiberSuspenseComponent.old";
...
import { prepareToHydrateHostInstance, prepareToHydrateHostTextInstance, prepareToHydrateHostSuspenseInstance, popHydrationState, resetHydrationState, getIsHydrating } from "./ReactFiberHydrationContext.old";
...
import { renderDidSuspend, renderDidSuspendDelayIfPossible, renderHasNotSuspendedYet, popRenderLanes, getRenderTargetTime, subtreeRenderLanes } from "./ReactFiberWorkLoop.old";
...
import { popCacheProvider, popRootCachePool, popCachePool } from "./ReactFiberCacheComponent.old";
...
if (supportsMutation) {
  ...
  updateHostContainer = function (current: null | Fiber, workInProgress: Fiber) {// Noop
  };
  ...
  updateHostComponent = function (current: Fiber, workInProgress: Fiber, type: Type, newProps: Props, rootContainerInstance: Container) {// Noop
  };

  updateHostText = function (current: Fiber, workInProgress: Fiber, oldText: string, newText: string) {// Noop
  };
}
...
export function completeSuspendedOffscreenHostContainer(current: Fiber | null, workInProgress: Fiber) {
    ...
    case SuspenseComponent:
      {
        ...
        if (nextDidTimeout && !prevDidTimeout) {
          ...
          if ((workInProgress.mode & ConcurrentMode) !== NoMode) {
            ...
            if (hasInvisibleChildContext || hasSuspenseContext(suspenseStackCursor.current, (InvisibleParentSuspenseContext as SuspenseContext))) {
