...
type RefreshHandler = (arg0: any) => Family | void;
...
// @ts-expect-error Flow gets confused by a WeakSet feature check below.
...
function scheduleFibersWithFamiliesRecursively(fiber: Fiber, updatedFamilies: Set<Family>, staleFamilies: Set<Family>) {
  if (__DEV__) {
    const {
      alternate,
      child,
