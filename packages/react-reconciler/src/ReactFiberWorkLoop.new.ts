...
import { onCommitRoot as onCommitRootDevTools, onPostCommitRoot as onPostCommitRootDevTools, isDevToolsPresent } from "./ReactFiberDevToolsHook.new";
...
export function batchedUpdates<A, R>(fn: (arg0: A) => R, a: A): R {
  const prevExecutionContext = executionContext;
...
export function flushControlled(fn: () => unknown): void {
  const prevExecutionContext = executionContext;
  executionContext |= BatchedContext;
  const prevTransition = ReactCurrentBatchConfig.transition;
  const previousPriority = getCurrentUpdatePriority();
...
function captureCommitPhaseErrorOnRoot(rootFiber: Fiber, sourceFiber: Fiber, error: unknown) {
  ...
  enqueueUpdate(rootFiber, update, (SyncLane as Lane));
  const eventTime = requestEventTime();
  const root = markUpdateLaneFromFiberToRoot(rootFiber, (SyncLane as Lane));
  ...
  while (fiber !== null) {
    if (fiber.tag === HostRoot) {
      ...
      if (typeof ctor.getDerivedStateFromError === 'function' || typeof instance.componentDidCatch === 'function' && !isAlreadyFailedLegacyErrorBoundary(instance)) {
        ...
        enqueueUpdate(fiber, update, (SyncLane as Lane));
        const eventTime = requestEventTime();
        const root = markUpdateLaneFromFiberToRoot(fiber, (SyncLane as Lane));
