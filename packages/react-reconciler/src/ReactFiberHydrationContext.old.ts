...
import type { Instance, TextInstance, HydratableInstance, SuspenseInstance, Container, HostContext } from "./ReactFiberHostConfig";
...
import { enableClientRenderFallbackOnHydrationMismatch, enableSuspenseServerRenderer } from "shared/ReactFeatureFlags";
    ...
    case SuspenseComponent:
      {
        if (enableSuspenseServerRenderer) {
          const suspenseInstance: null | SuspenseInstance = canHydrateSuspenseInstance(nextInstance);
...
function throwOnHydrationMismatchIfConcurrentMode(fiber: Fiber) {
  if (enableClientRenderFallbackOnHydrationMismatch && (fiber.mode & ConcurrentMode) !== NoMode) {
...
function tryToClaimNextHydratableInstance(fiber: Fiber): void {
  ...
  if (!tryHydrate(fiber, nextInstance)) {
    ...
    const prevHydrationParentFiber: Fiber = (hydrationParentFiber as any);
...
export { warnIfHydrating, enterHydrationState, getIsHydrating, reenterHydrationStateFromDehydratedSuspenseInstance, resetHydrationState, tryToClaimNextHydratableInstance, prepareToHydrateHostInstance, prepareToHydrateHostTextInstance, prepareToHydrateHostSuspenseInstance, popHydrationState };