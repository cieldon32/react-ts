...
import type { SuspenseState, SuspenseListRenderState, SuspenseListTailMode } from "./ReactFiberSuspenseComponent.old";
import type { SuspenseContext } from "./ReactFiberSuspenseContext.old";
import type { OffscreenProps, OffscreenState } from "./ReactFiberOffscreenComponent";
import type { Cache, CacheComponentState, SpawnedCachePool } from "./ReactFiberCacheComponent.old";
...
import { pushProvider, propagateContextChange, lazilyPropagateParentContextChanges, propagateParentContextChangesToDeferredTree, checkIfContextChanged, readContext, prepareToReadContext, scheduleWorkOnParentPath } from "./ReactFiberNewContext.old";
...
import { getIsHydrating, enterHydrationState, reenterHydrationStateFromDehydratedSuspenseInstance, resetHydrationState, tryToClaimNextHydratableInstance, warnIfHydrating } from "./ReactFiberHydrationContext.old";
...
import { retryDehydratedSuspenseBoundary, scheduleUpdateOnFiber, renderDidSuspendDelayIfPossible, markSkippedUpdateLanes, getWorkInProgressRoot, pushRenderLanes, getExecutionContext, RetryAfterError, NoContext } from "./ReactFiberWorkLoop.old";
import { setWorkInProgressVersion } from "./ReactMutableSource.old";
import { requestCacheFromPool, pushCacheProvider, pushRootCachePool, CacheContext, getSuspendedCachePool, restoreSpawnedCachePool, getOffscreenDeferredCachePool } from "./ReactFiberCacheComponent.old";
...
function updateForwardRef(current: Fiber | null, workInProgress: Fiber, Component: any, nextProps: any, renderLanes: Lanes) {
...
function updateMemoComponent(current: Fiber | null, workInProgress: Fiber, Component: any, nextProps: any, renderLanes: Lanes): null | Fiber {
  if (current === null) {
...
function updateSimpleMemoComponent(current: Fiber | null, workInProgress: Fiber, Component: any, nextProps: any, renderLanes: Lanes): null | Fiber {
  // TODO: current can be non-null here even if the component
...
function updateOffscreenComponent(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes) {
...
function reconcileOffscreenHostContainer(currentOffscreen: Fiber | null, offscreen: Fiber, isHidden: boolean, children: any, renderLanes: Lanes) {
...
function updateCacheComponent(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes) {
  if (!enableCache) {
    return null;
  }
...
function updateFragment(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes) {
  const nextChildren = workInProgress.pendingProps;
  reconcileChildren(current, workInProgress, nextChildren, renderLanes);
  return workInProgress.child;
}

function updateMode(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes) {
  const nextChildren = workInProgress.pendingProps.children;
  reconcileChildren(current, workInProgress, nextChildren, renderLanes);
  return workInProgress.child;
}

function updateProfiler(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes) {
    ...
    if (supportsHydration) {
      ...
      if (mutableSourceEagerHydrationData != null) {
        for (let i = 0; i < mutableSourceEagerHydrationData.length; i += 2) {
          const mutableSource = ((mutableSourceEagerHydrationData[i] as any) as MutableSource<any>);
...
function shouldRemainOnFallback(suspenseContext: SuspenseContext, current: null | Fiber, workInProgress: Fiber, renderLanes: Lanes) {
...
function updateSuspenseComponent(current, workInProgress, renderLanes) {
  ...
  if (didSuspend || shouldRemainOnFallback(suspenseContext, current, workInProgress, renderLanes)) {
    ...
    if (current === null || (current.memoizedState as null | SuspenseState) !== null) {
    ...
    if (prevState !== null) {
      ...
      if (enableSuspenseServerRenderer) {
        ...
        if (dehydrated !== null) {
          ...
          } else if ((workInProgress.memoizedState as null | SuspenseState) !== null) {
      ...
      if (showFallback) {
        ...
        const primaryChildFragment: Fiber = (workInProgress.child as any);
        const prevOffscreenState: OffscreenState | null = (current.child as any).memoizedState;
...
function mountWorkInProgressOffscreenFiber(offscreenProps: OffscreenProps, mode: TypeOfMode, renderLanes: Lanes) {
  // The props argument to `createFiberFromOffscreen` is `any` typed, so we use
  // this wrapper function to constrain it.
  return createFiberFromOffscreen(offscreenProps, mode, NoLanes, null);
}

function updateWorkInProgressOffscreenFiber(current: Fiber, offscreenProps: OffscreenProps) {
...
function updateSuspenseFallbackChildren(current, workInProgress, primaryChildren, fallbackChildren, renderLanes) {
  ...
  workInProgress.child !== currentPrimaryChildFragment) {
    ...
    if (enablePersistentOffscreenHostContainer && supportsPersistence) {
      ...
      const offscreenContainer: Fiber = (primaryChildFragment.child as any);
...
function retrySuspenseComponentWithoutHydrating(current: Fiber, workInProgress: Fiber, renderLanes: Lanes) {
...
function mountDehydratedSuspenseComponent(workInProgress: Fiber, suspenseInstance: SuspenseInstance, renderLanes: Lanes): null | Fiber {
  // During the first pass, we'll bail out and not drill into the children.
  // Instead, we'll leave the content in place and try to hydrate it later.
  if ((workInProgress.mode & ConcurrentMode) === NoMode) {
    if (__DEV__) {
      console.error('Cannot hydrate Suspense in legacy mode. Switch from ' + 'ReactDOM.hydrate(element, container) to ' + 'ReactDOM.hydrateRoot(container, <App />)' + '.render(element) or remove the Suspense components from ' + 'the server rendered components.');
    }
...
function updateDehydratedSuspenseComponent(current: Fiber, workInProgress: Fiber, suspenseInstance: SuspenseInstance, suspenseState: SuspenseState, renderLanes: Lanes): null | Fiber {
...
function propagateSuspenseContextChange(workInProgress: Fiber, firstChild: null | Fiber, renderLanes: Lanes): void {
...
function validateTailOptions(tailMode: SuspenseListTailMode, revealOrder: SuspenseListRevealOrder) {
  if (__DEV__) {
...
function validateSuspenseListChildren(children: unknown, revealOrder: SuspenseListRevealOrder) {
  if (__DEV__) {
...
function initSuspenseListRenderState(workInProgress: Fiber, isBackwards: boolean, tail: null | Fiber, lastContentRow: null | Fiber, tailMode: SuspenseListTailMode): void {
...
function updateSuspenseListComponent(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes) {
  ...
  let suspenseContext: SuspenseContext = suspenseStackCursor.current;
  const shouldForceFallback = hasSuspenseContext(suspenseContext, (ForceSuspenseFallback as SuspenseContext));
