 ...
 */
export function getValueForProperty(node: Element, name: string, expected: unknown, propertyInfo: PropertyInfo): unknown {
  if (__DEV__) {
 ...
 * @param {*} value
 */
export function setValueForProperty(node: Element, name: string, value: unknown, isCustomComponentTag: boolean) {
  ...
  if (enableCustomElementPropertySupport && isCustomComponentTag && name in (node as any)) {
  ...
  if (isCustomComponentTag || propertyInfo === null) {
    if (isAttributeNameSafe(name)) {
      ...
      if (value === null) {
        ...
        node.setAttribute(attributeName, enableTrustedTypesIntegration ? (value as any) : '' + (value as any));
      }
    }
