...
export function createRoot(container: Container, options?: CreateRootOptions): RootType {
  ...
  if (options !== null && options !== undefined) {
    if (__DEV__) {
      if ((options as any).hydrate) {
        console.warn('hydrate through createRoot is deprecated. Use ReactDOM.hydrateRoot(container, <App />) instead.');
      }
    }
  ...
  const root = createContainer(container, ConcurrentRoot, false, null, isStrictMode, concurrentUpdatesByDefaultOverride, identifierPrefix);
...
export function hydrateRoot(container: Container, initialChildren: ReactNodeList, options?: HydrateRootOptions): RootType {
