...
function updateOptions(node: HTMLSelectElement, multiple: boolean, propValue: any, setDefaultSelected: boolean) {
  type IndexableHTMLOptionsCollection = HTMLOptionsCollection & Record<number, HTMLOptionElement>;
...
export function postMountWrapper(element: Element, props: Record<string, any>) {
  const node = ((element as any) as SelectWithWrapperState);
...
export function postUpdateWrapper(element: Element, props: Record<string, any>) {
  const node = ((element as any) as SelectWithWrapperState);
